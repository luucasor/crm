...
class Vtiger_IndexAjax_View extends Vtiger_Index_View {
	...
	//@@CUSTOM
	public function getDatesOfFilter($filterDashboard){
		if ($filterDashboard['initialdate'] != null && $filterDashboard['finaldate'] != null) {
			$dates['start'] = $filterDashboard['initialdate'];
			$dates['end'] = $filterDashboard['finaldate'];
		}
		return $dates;
	}

	public function getSmownerId($request, $filterDashboard, $currentUser){
		$smownerid = $request->get('smownerid');

		if($filterDashboard){
			$idUser = $currentUser->getIdByName($filterDashboard['responsible']);
			if(!empty($idUser))
				$smownerid = $idUser;
		}
		return $smownerid;
