...
{strip}
		...
		{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
			{if !$FIELD_MODEL->isViewableInDetailView()}
			 ...
			 {if $FIELD_MODEL->get('uitype') eq "83"}
				...
				{if $tax.check_value eq 1}
					...
					<label id="label_{$FIELD_NAME}" class='muted pull-right marginRight10px'>{vtranslate($tax.taxlabel, $MODULE)}(%)</label>
					</td>
					 <td class="fieldValue {$WIDTHTYPE}">
						 <span name="{$FIELD_NAME}" class="value">
							 {$tax.percentage}
						 </span>
					 </td>
				{/if}
			...
			{else if $FIELD_MODEL->get('uitype') eq "69" || $FIELD_MODEL->get('uitype') eq "105"}
				{if $COUNTER neq 0}
					...
					{/if}
				{/if}
				<td class="fieldLabel {$WIDTHTYPE}"><label id="label_{$FIELD_NAME}" class="muted pull-right marginRight10px">{vtranslate({$FIELD_MODEL->get('label')},{$MODULE_NAME})}</label></td>
				<td name="{$FIELD_NAME}" class="fieldValue {$WIDTHTYPE}">
			...
			{else}
				...
				{else}
				 ...
				 <td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
					 <span name="{$FIELD_NAME}" class="value" data-field-type="{$FIELD_MODEL->getFieldDataType()}" {if $FIELD_MODEL->get('uitype') eq '19' or $FIELD_MODEL->get('uitype') eq '20' or $FIELD_MODEL->get('uitype') eq '21'} style="white-space:normal;" {/if}>
    ...
    <table id="veiculos-table" style="margin-bottom:15px;margin-right:40px;" class="table table-bordered blockContainer showInlineTable equalSplit">
        <thead>
            <tr>
                <th colspan="5">Informações Veículos</th>
            </tr>
            <tr>
                <th>Tipo</th>
                <th>Modelo</th>
        ...
        <tbody>
            {foreach key=key item=item from=$VEHICLES}
                <tr>
					...
					<td style='width: 100px;'><a href="http://crm.quatenusonline.com.br/crm/index.php?module=Potentials&view=Detail&record={$item->getIdPotential()}">{$item->getIdPotential()}</a></td>
                </tr>
            {/foreach}
        </tbody>
        <tfoot>
            <tr>
                <td id="nreg" style="text-align: left;" class="fieldLabel" colspan="5">{count($VEHICLES)} Veículo(s)</td>
            </tr>
        </tfoot>
    </table>
{/strip}
